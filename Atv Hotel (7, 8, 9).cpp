...
void fArmazenadados (int l, int c, tipoQuarto tab[20][14]);
int fVerificaCPF (char ID[14], int l, int c, tipoQuarto tab[20][14]);
void fPorcentagemReOc (tipoQuarto tab[20][14]);
void fclear();

main()
{	
	tipoQuarto tabe[20][14]; //inicializa o vetor Bidimensional que mostra o mapa de quartos
	    	...
	    	{	
				...
				case 7: 
				    ...
				    if((lr<=20 && lr>=1) && (cr<=14 && cr>=1)) //Verifica se o andar e o quarto estao dentro dos parametros permitidos
			    	{
					    ...
					    if(tabe[lr][cr].situacao == 'O')
					    {
						    printf("\n------O Quarto %d do andar %d esta ocupado pela pessoa cuja os dados sao:------\n",cr+1,lr+1);
						    printf("CPF: %s\n", tabe[lr][cr].cliente.CPF);
						    printf("Nome: %s\n", tabe[lr][cr].cliente.Nome);
						    printf("CEP:%s\nEndereco:%s\nMunicipio:%s\nEstado:%s\n", tabe[lr][cr].cliente.endereco.cep, tabe[lr][cr].cliente.endereco, tabe[lr][cr].cliente.endereco.municipio, tabe[lr][cr].cliente.endereco.estado);
						    printf("Celular: %s\n", tabe[lr][cr].cliente.telefone);
						    printf("Email: %s\n", tabe[lr][cr].cliente.email);
						    repet++;
					    }
					    
					    else if(tabe[lr][cr].situacao == 'R')
					    {
...
int fVerificaCPF (char ID[14], int l, int c, tipoQuarto tab[20][14])
{
	...
	while (ID[i] != '\0' && tab[l][c].cliente.CPF[i] != '\0') 
	{
        if (ID[i] != tab[l][c].cliente.CPF[i]) 
		{
            return 0;
            break;
        }
...
void fPorcentagemReOc (tipoQuarto tab[20][14])
{
	int reservado=0, ocupado=0, Total=0;
	float T1, T2, T3;
	...
	for(int l = 0; l <20; l++) 
	{
		for (int c = 0; c < 14; c++)
		{
			if(tab[l][c].situacao=='O') ocupado++;
			else if (tab[l][c].situacao=='R')reservado++;	
		}
	}
	
	Total = 20*14;
	T1 = ocupado/(Total/100.0);
	T2 = reservado/(Total/100.0);
	T3 = 100 - T2 - T1;
	
	printf("\n-----------------------------------------");
	printf("\n--> Taxa de ocupacao: %.2f", T1);
	printf("\n--> Taxa de reservas: %.2f", T2);
	printf("\n--> Taxa de quartos livres: %.2f", T3);
	printf("\n-----------------------------------------\n");
}
